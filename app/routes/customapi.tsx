...
async function updatePhoneCustomerSms(admin: AdminApiContext, phone: string) {
  const customerRequest = await admin.graphql(CustomerGQL, {
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (!admin) return jsonResponse('auth fail')
  ...
  //- 根据 email 判断当前客户是否存在
  const customerRequest = await admin.graphql(CustomerGQL, {
    variables: { identifier: { emailAddress: formData.get('email') } },
  })
  const customerReqJson = await customerRequest.json()
  const customerId = customerReqJson.data?.customer?.id
  if (!customerId) {
      ...
      variables: {
        input: {
          ...
          ...(formData.has('phone')
            ...
            : {}),
          ...(formData.has('tags')
            ? { tag: formData.get('tags')?.toString().split(',') }
            : { tags: ['newsletter'] }),
        },
      },
    })
